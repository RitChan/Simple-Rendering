...
int main() {
    ...
    shader.model_ = create_rotation_y(M_PI_4 + 0.2) * shader.model_;
    // auto view = create_view_matrix({0, 1, 5}, {0, -0.2, -1}, {0, 1, 0});
    auto view = create_view_matrix({0, 1, 3}, {0, -0.5, -1}, {0, 1, 0});
    auto project =
    ...
    shader.set_texture_with_mipmap(texture, 7);
